...
// +++ MPrimeBound +++ (also in ModularFunctions.m)
...
Modular:= function(A,SA,B,SB,C,m);
     ...
     if B ne 1 then
        D := B; SD := SB;    // Choose D as A or B, depending on which is not 1
     else D := A; SD := SA;
     end if;

     RadD := Rad(D,1);
     Badp := Set(PrimeFactors(EulerPhi(RadD)));
     Badp := {p : p in Badp | p ne 2};

     print "Initial bad primes are", Badp;

     if Badp ne {} and Max(Badp) gt 7 then 
        assert Valuation(D,2) eq 1; // sanity check
     ...
     for p in Badp do
         ...
         print #lsp, "auxilliary primes found";
